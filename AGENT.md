...
- В sidebar обычное состояние строки папки показывает только chevron, folder icon и имя. Действия папки открываются через hover/focus меню `⋯`; проекты внутри папок являются прямыми ссылками без собственного меню, потому что их действия доступны в рабочей области выбранного проекта. Не возвращайте отдельные action-иконки в строку без сильной UX-причины. Формы создания проекта, экспорта и настройки ping должны открываться в modal, а не встраиваться внутрь sidebar.
...
- Пользовательские столбцы хранятся как metadata в `custom_fields`, а значения - в JSON-поле `ip_addresses.custom_values`. Управление выполняется атомарно через `replace_custom_fields`: создание, имя, тип, позиция и удаление сохраняются одной транзакцией.
...
- Права обычных пользователей:
  ...
  - `can_manage_columns` - создание, переименование, смена типа, перестановка и удаление пользовательских столбцов в проектах.
...
- AJAX должен применять только последний запрошенный ответ. Redirect на login выполняет полную навигацию, а не вставку формы входа в таблицу; ошибки чтения ответа используют полную навигацию. Browser Back не должен затирать dirty drawer; модифицированные клики по ссылкам сохраняют обычное поведение браузера.
...
- Action bar проекта должен оставаться компактным: отдельными кнопками показывайте только `Show hidden` и `Columns`, а менее частые действия (`Ping`, `Change history`, `Export project`, `Project settings`, `Delete project`) держите в верхнем меню `⋯` с учетом текущей ролевой модели.
- Главная страница `/` должна показывать empty state выбора проекта, если проект явно не открыт. Не возвращайте автоматический редирект на первый проект после login, удаления проекта или перехода на `/`.
...
- Фронтенд-защита от потери данных находится в `app/static/app.js`: изменения делаются только в правом drawer, кнопка `Save` появляется после `Edit`, переход к другой строке предупреждает о несохраненных данных.
...
- `app/templates/project.html` - рабочая область проекта, таблица IP, модальное управление столбцами и настройки проекта по правам.
...
- `tests/test_custom_fields.py` - тесты ключей и атомарного управления пользовательскими полями, очистки значений и modal UI.
...
`bandit==1.9.4` поддерживает AST Python 3.14; проверяйте отсутствие внутренних ошибок анализа и пропущенных файлов. Не отключайте правила целиком ради зеленого отчета: подписи полей пароля в переводах, пустые необязательные пароли и импорт `ParseError` оценивайте вручную. CSRF-разметка строится через константный `Markup(...).format(...)` с автоматическим экранированием аргументов.

После ревизии закреплены `starlette==1.3.1`, `pydantic-settings==2.14.2`, `python-multipart==0.0.31`: это адресные обновления по результатам pip-audit. При следующих обновлениях повторяйте тесты форм/CSRF, файлового импорта и login; одной успешной установки пакетов недостаточно.
...
- Кнопка `Save` находится только в drawer и скрыта до `Edit`. Не возвращайте отдельную колонку действий или inline-редактирование в таблицу.
- Столбец `№` скрыт в `app/templates/_project_table.html` через `show_number_column = false`. Если включаете его обратно, проверьте комментарий рядом с флагом и задайте sticky offset `.locked-col` по ширине `.col-number`.
- `Settings` использует `extra="ignore"`, поскольку общий `.env` содержит также `APP_PORT` и `POSTGRES_*` для Compose. Сохраняйте валидацию объявленных настроек; изменение этого контракта проверяйте тестом общего dotenv.
...
- Заголовок определяется по первой непустой строке; допускаются пробелы/регистр и пустые конечные колонки, но не пропуски внутри заголовка. Строки валидируются по мере чтения, CSV использует strict-парсер, XLSX закрывается в `finally`, ошибки чтения преобразуются в `CSVImportError`.
- Для ручного создания проекта поле CIDR обязательно; для импорта CSV/XLSX CIDR не требуется и рассчитывается автоматически.
- Модальное окно `Columns` работает как черновик: `Save` отключена до изменения, `Cancel` всегда доступна и восстанавливает исходный список. Порядок строк становится `custom_fields.position`.
- При удалении custom field сервис удаляет его ключ из `ip_addresses.custom_values`; не оставляйте скрытые значения. При смене типа на `number` или `date` сначала проверяются все непустые значения, несовместимое значение блокирует всю транзакцию без частичного сохранения.
...
- Не допускайте исполнения пользовательских значений как формул: XLSX принудительно сохраняет строковые ячейки с `data_type="s"`; CSV экранирует опасные формульные/управляющие префиксы апострофом. Это меняет только экспорт, не БД. CSV-апостроф не удаляется автоматически при обратном импорте; для точного переноса используйте XLSX.
...
- `ping_address` обязан завершать и дожидаться дочернего процесса при timeout/cancellation, иначе ограничение параллелизма обходится оставшимися процессами. Ошибка запуска (`OSError`, в том числе запрет выполнения) означает ошибку probe, а не недоступный IP.
...
- Обычные пользователи без `can_manage_columns` не видят кнопку `Columns`, а backend также отклоняет POST управления конфигурацией.
...
- Добавить мониторинг очереди ping-задач: прогресс, последняя проверка, ошибки и история выполнений.
- Добавить heartbeat, проверку владельца завершаемой задачи и PostgreSQL-тесты конкурентного claim/requeue/enqueue.
- Добавить сохраненные фильтры и фильтры пользовательских столбцов в компактную панель таблицы.
- Добавить preview импорта до создания проекта: CIDR, число записей, сопоставление колонок, список ошибок.
- Добавить optimistic locking IP-записей с отображением конфликта в drawer.
- Добавить browser-регрессии и полный lock транзитивных зависимостей в CI.
- Заменить `datetime.utcnow()` единым UTC-контрактом без смешивания naive/aware дат в существующей БД.

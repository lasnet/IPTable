...
- Фронтенд-защита от потери данных находится в `app/static/app.js`: измененная строка подсвечивается красным, кнопка `OK` появляется только у dirty row, переход к другой строке предупреждает о несохраненных данных.
...
- `app/static/app.js` - dirty-state таблицы, предупреждение о несохраненных строках и показ кнопки сохранения.
...
- Кнопка `OK` должна быть отдельной колонкой, не внутри `Статус (Ping)`.

...
- В админ-панели форма создания пользователя находится в левой карточке, список пользователей - в правой карточке, а редактирование пользователя открывается в modal `Edit user`. Не возвращайте большие inline-формы редактирования внутрь списка пользователей.
...
- CSV импортируется с разделителем `;`, XLSX - из первой таблицы workbook. Поддерживаются колонки в порядке `ip`, `hostname`, `os`, `type`, `comment`; строка заголовка поддерживается, но не обязательна. Подсеть проекта рассчитывается по минимальной сети, которая покрывает импортированные IPv4-адреса.
...
- Header проекта содержит компактный summary-блок, а таблица IP имеет right drawer деталей строки. Таблица должна оставаться read-only; редактирование, `Clear` и `View history` выполняются из drawer, не возвращайте постоянные кнопки или меню `⋯` в строки без сильной UX-причины.
- Action bar проекта должен оставаться компактным: отдельными кнопками показывайте только `Show hidden` и `+ Column`, а менее частые действия (`Ping`, `Change history`, `Export project`, `Project settings`, `Delete project`) держите в верхнем меню `⋯` с учетом текущей ролевой модели.
- UI должен показывать явные состояния: skeleton таблицы при AJAX-загрузке, skeleton дерева при обычной навигации/submit, empty state для папки без проектов и project empty state при скрытых пустых строках и отсутствии заполненных активов.
- Ошибки импорта CSV/XLSX должны оставаться понятными для пользователя. Если доступен контекст строки/колонки, включайте его в текст ошибки и сохраняйте RU/EN перевод через `translate_error_message`.
...
- Импорт CSV/XLSX принимает файлы с заголовком `ip;hostname;os;type;comment` и без него, но отклоняет не-IPv4 адреса, дубли IP, network/broadcast адреса, лишние заполненные колонки после `comment`, слишком большую рассчитанную подсеть и слишком большой файл.

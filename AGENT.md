...
- `app/static/app.js` - dirty-state таблицы, предупреждение о несохраненных строках, показ кнопки сохранения, включение/отключение поля пароля в формах экспорта и прокрутка к найденной IP-строке.

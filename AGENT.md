...
- Ping-worker защищен от флуда: полный проход идет последовательно по проектам, внутри проекта адреса проверяются пакетами. Управляющие настройки: `PING_CONCURRENCY`, `PING_BATCH_SIZE`, `PING_BATCH_PAUSE_SECONDS`, `PING_PROJECT_PAUSE_SECONDS`.
...
- Столбец `№` скрыт CSS-блоком в `app/static/styles.css`. В этом блоке есть комментарий, что нужно раскомментировать/вернуть, если столбец снова понадобится.
...
- Добавить расписания ping-проверок по папкам/проектам и окно обслуживания.

...
- В sidebar обычное состояние строки папки показывает только chevron, folder icon и имя. Действия папки/проекта открываются через hover/focus меню `⋯`; не возвращайте отдельные action-иконки в строку без сильной UX-причины.

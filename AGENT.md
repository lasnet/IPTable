...
- Базовые поля IP-записи: порядковый номер, IP-адрес, hostname, OS, type, comment, tags.
...
- Пустые IP-строки по умолчанию скрыты. Ping-статус не считается заполнением строки; скрытие смотрит на hostname, OS, type, comment, tags и custom fields.
...
- CSV импортируется с разделителем `;`, XLSX - с первой строки заголовков. Поддерживаются колонки `ip`, `hostname`, `os`, `type`, `comment` и опциональная `tags`; подсеть проекта рассчитывается по минимальной сети, которая покрывает импортированные IPv4-адреса.
...
- Массовое редактирование работает через `/projects/{project_id}/addresses/bulk`: выбранные чекбоксами строки получают только заполненные в форме массового редактирования поля.
...
- `app/services/assets.py` - нормализация тегов активов.
...
- `app/static/app.js` - dirty-state таблицы, предупреждение о несохраненных строках, AJAX-пагинация таблицы, bulk-select, включение/отключение поля пароля в формах экспорта и прокрутка к найденной IP-строке.
...
- теги активов;
- массовое редактирование выбранных IP-записей;
...
- Добавить фильтры таблицы по тегам, статусу ping и типу актива.

...
9. Заполните `hostname`, `OS`, `type` или `comment` у нескольких адресов.
...
- CSV/XLSX не импортируется: проверьте порядок колонок `ip;hostname;os;type;comment`, разделитель `;` для CSV, отсутствие дублей IP и network/broadcast адресов. Заголовок первой строкой допускается, но не обязателен.

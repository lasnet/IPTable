...
Первый версионированный релиз готовится: наличие CI не означает, что образ уже
опубликован или RC прошел проверку Docker. Production использует `deploy/compose.yml`
...
- Название проекта используется как человекочитаемое имя подсети и отображается в заголовке рабочей области, поиске и форме редактирования; CIDR остается сетевым идентификатором.
- Проекты в боковой панели внутри папки сортируются по сетевому адресу CIDR, а не по времени создания.
- Действия папок в боковой панели спрятаны в hover-меню `⋯`, чтобы дерево оставалось компактным. Проекты внутри папок отображаются как прямые ссылки без собственного меню, потому что их действия доступны в рабочей области выбранного проекта. Создание проекта, экспорт и настройка ping открываются в модальных окнах, а не внутри sidebar.
...
- Summary-блок проекта с показателями `Total IPs`, `Used`, `Empty / Hidden`, `Ping OK`, `Ping NO`.
- Компактный action bar проекта: отдельно остаются `Show hidden` и `Columns`, а `Ping`, история, экспорт и настройки проекта находятся в меню `⋯` с учетом прав пользователя.
- Если проект не выбран, рабочая область показывает пустое состояние с просьбой выбрать проект, а не открывает первую подсеть автоматически.
- Right drawer деталей IP/host по клику на строку таблицы. Редактирование, `Clear` и `View history` выполняются из drawer, сама таблица остается read-only.
- Управление пользовательскими столбцами в модальном окне `Columns`: создание, переименование, смена типа, изменение порядка и удаление. Изменения применяются одной кнопкой `Save`; удаление столбца очищает его значения из IP-записей.
- Авторизация пользователей через страницу входа.
- CSRF-защита всех POST-форм и ограничение частоты попыток входа.
- Админ-панель `/admin/users` для создания обычных пользователей и назначения прав: создание остается в левой карточке, список пользователей - в правой, редактирование открывается в modal.
...
- Импорт проекта из CSV с разделителем `;` или XLSX с колонками `ip`, `hostname`, `os`, `type`, `comment`; строка заголовка поддерживается, но не обязательна.
...
- Редактирование IP в правом drawer: кнопка `Save` появляется после `Edit`; несохраненные изменения защищены предупреждением, в том числе при переходе назад в AJAX-пагинации.
- История изменений IP-записей: кто, когда, какой IP и какое поле изменил. Историю можно смотреть по проекту или по всей папке.
...
- При истечении сессии AJAX-пагинация переводит на страницу входа; запоздалый ответ не заменяет более свежую страницу таблицы.
...
- Аккуратные UI-состояния: skeleton таблицы при AJAX-загрузке, skeleton дерева при переходах, empty state для пустых папок/проектов и отключение кнопок во время отправки форм.
- Ошибки импорта CSV/XLSX показываются понятным сообщением `Import error` / `Ошибка импорта` с указанием строки и колонки, если это известно.
...
Приложение читает общий `.env` вместе с Docker Compose: переменные `APP_PORT` и `POSTGRES_*` не считаются ошибкой конфигурации. Значения известных настроек приложения продолжают проходить валидацию; неизвестные имена игнорируются, поэтому проверяйте их написание по `.env.example`.
...
- `postgres` - база данных, перезапускается автоматически после reboot Docker/сервера.
- `migrate` - разово выполняет `alembic upgrade head`.
- `web` - веб-интерфейс FastAPI, перезапускается автоматически.
- `worker` - отдельный ping-worker с ICMP capability `NET_RAW`, перезапускается автоматически.
...
python -m ruff check .
semgrep --metrics=off --config p/python app scripts
checkov -f Dockerfile --framework dockerfile --skip-download --compact
checkov -f docker-compose.yml --framework yaml --skip-download --compact
...
При ревизии зависимостей обновлены `starlette` до `1.3.1`, `pydantic-settings` до `2.14.2`, `python-multipart` до `0.0.31` для устранения найденных `pip-audit` уязвимостей. После обновления исходников пересоберите Docker-образы или выполните `pip install -r requirements-dev.txt` в локальном окружении; отдельно запускать новые миграции для этих исправлений не требуется.
...
7. Для импорта нажмите `Импортировать из CSV/XLSX` в форме создания проекта и загрузите файл с колонками `ip;hostname;os;type;comment`. Заголовок можно добавить первой строкой, но он не обязателен.
8. Откройте проект и нажмите `Показать скрытые`, если проект новый и все строки пока пустые.
9. Нажмите на строку, затем `Edit` в правом drawer и заполните `hostname`, `OS`, `type` или `comment`.
10. Сохраните через `Save`. Повторите редактирование и проверьте предупреждение при попытке уйти с несохраненными изменениями.
11. Откройте `История` в проекте и убедитесь, что изменение строки записалось.
12. Проверьте экспорт проекта и папки через соответствующие меню `⋯`.
13. Проверьте фильтры таблицы по `Ping`, `Type` и `OS`.
14. Откройте `Columns`, добавьте два пользовательских столбца, поменяйте их порядок и тип, затем сохраните. Повторно откройте окно и проверьте сохраненный порядок. Удаление применяется только после `Save`, а `Cancel` отбрасывает черновик.
15. Проверьте поиск через верхнюю форму.
16. Для проекта или папки откройте расписание `Ping`, измените интервал или включите `Запустить сейчас`.
17. Проверьте `docker compose logs -f worker`: worker должен забирать задачи из очереди.
18. Если задан `INTEGRATION_API_TOKEN`, проверьте `GET /api/v1/projects` с заголовком `X-API-Key`.
19. Выполните `scripts/backup_postgres.sh` и убедитесь, что появился файл в `backups/`.
20. Откройте `http://localhost:8000/health` и убедитесь, что вернулся статус `ok`.
...
- Смена типа пользовательского столбца на `Number` или `Date` разрешается только тогда, когда все его непустые значения совместимы с новым типом. Ошибка показывает первый проблемный IP; исправьте значение в drawer и повторите изменение типа.
- Удаление пользовательского столбца после `Save` окончательно удаляет его значения из `custom_values`. Используйте `Cancel`, если изменение было сделано ошибочно и еще не сохранено.
...
- CSV/XLSX не импортируется: проверьте порядок колонок `ip;hostname;os;type;comment`, разделитель `;` для CSV, отсутствие дублей IP и network/broadcast адресов. Заголовок в первой непустой строке допускается, но не обязателен. Пустые колонки внутри заголовка недопустимы: они смещают данные. Незакрытые кавычки CSV и поврежденный XLSX возвращают ошибку импорта.
- В CSV-экспорте появился апостроф перед значением: это защита от выполнения формул в табличном редакторе для значений с `=`, `+`, `-`, `@` и управляющими префиксами. База не меняется, но при повторном импорте CSV апостроф останется частью данных. Для точного переноса таких значений используйте XLSX: ячейки сохраняются как текст без дополнительного префикса. Не преобразуйте их в формулы при открытии файла.
...
- После reboot сервера web/worker постоянно рестартуют, а `postgres` остановлен: проверьте, что в `docker-compose.yml` у `postgres` есть `restart: unless-stopped`, затем выполните `docker compose up -d`. `migrate` - одноразовый сервис и после успешного `Exited (0)` сам не перезапускается.
- Изменили `.env`, но настройка не применилась: если контейнеры запускались через `docker compose start`, старые environment values сохраняются. Выполните `docker compose up -d --force-recreate web worker` или полный `docker compose up -d --build`, чтобы web/worker получили новые значения.
- Сессия не завершилась ровно через `SESSION_IDLE_TIMEOUT_SECONDS`: это idle-timeout, он продлевается при каждом авторизованном запросе. Если страница просто открыта без запросов, визуально она изменится только при следующем действии или обновлении страницы.
...
- Добавить страницу состояния ping-очереди: прогресс, время последней проверки, ошибки worker и история выполнений.
- Добавить heartbeat и проверку владельца ping-задачи при завершении, чтобы долгий живой worker не конфликтовал с повторно выданной задачей; проверить конкурентные сценарии на PostgreSQL.
- Добавить сохраненные фильтры и фильтры пользовательских столбцов, сохранив компактную панель над таблицей.
- Добавить preview импорта: CIDR, число строк, примеры сопоставления колонок и ошибки до подтверждения создания проекта.
- Добавить защиту от перезаписи IP-записи двумя пользователями одновременно с понятным сообщением о конфликте в drawer.
- Добавить постоянные browser-тесты пагинации, drawer и мобильной верстки в CI; полную фиксацию транзитивных зависимостей и регулярный аудит обновлений.
- Перейти с устаревшего `datetime.utcnow()` на единый UTC-контракт с тестами совместимости существующих дат БД.
...
- Добавить экспорт истории изменений.

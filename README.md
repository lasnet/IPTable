...
- Защита от потери несохраненных изменений: измененная строка подсвечивается красным, а кнопка `OK` появляется только для нее.
...
7. Убедитесь, что измененная строка подсвечивается красным и кнопка `OK` появляется только у нее.
8. Вернитесь к режиму скрытия пустых строк и проверьте, что отображаются только строки с заполненными данными.
9. Проверьте поиск через верхнюю форму.
10. Откройте `http://localhost:8000/health` и убедитесь, что вернулся статус `ok`.

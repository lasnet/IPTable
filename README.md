...
12. Для проекта или папки откройте расписание `Ping`, измените интервал или включите `Запустить сейчас`.
13. Проверьте `docker compose logs -f worker`: worker должен забирать задачи из очереди.
14. Откройте `http://localhost:8000/health` и убедитесь, что вернулся статус `ok`.

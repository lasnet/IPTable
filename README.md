...
- Редактирование пользователей администратором: логин, пароль, имя, фамилия, описание и права. Администратора из `.env` удалить нельзя, а его логин и права управляются `.env`.
...
- Фоновая проверка доступности IP-адресов через ICMP ping раз в 60 минут с защитой от флуда: проекты проверяются последовательно, адреса идут пакетами с ограниченным параллелизмом и паузами.
...
- Для защиты сети от всплесков ICMP используются `PING_CONCURRENCY`, `PING_BATCH_SIZE`, `PING_BATCH_PAUSE_SECONDS` и `PING_PROJECT_PAUSE_SECONDS`.
...
10. Проверьте экспорт проекта через кнопку `Экспорт`, а экспорт папки через иконку экспорта рядом с папкой.
11. Проверьте поиск через верхнюю форму.
12. Откройте `http://localhost:8000/health` и убедитесь, что вернулся статус `ok`.
...
- Вынести ping-worker в отдельный сервис/очередь при росте нагрузки и добавить расписания проверки по папкам/проектам.
...
- Добавить резервное копирование PostgreSQL и инструкцию восстановления.

...
- Импорт проекта из CSV с разделителем `;` или XLSX с колонками `ip`, `hostname`, `os`, `type`, `comment`.
...
7. Для импорта нажмите `Импортировать из CSV/XLSX` в форме создания проекта и загрузите файл с заголовком `ip;hostname;os;type;comment`.
...
- CSV/XLSX не импортируется: проверьте заголовок `ip;hostname;os;type;comment`, разделитель `;` для CSV, отсутствие дублей IP и network/broadcast адресов.

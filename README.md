...
- Массовое редактирование выбранных IP-записей на текущей странице таблицы.
...
6. Для импорта нажмите `Импортировать из CSV/XLSX` в форме создания проекта и загрузите файл с заголовком `ip;hostname;os;type;comment` или дополнительной колонкой `tags`.
7. Откройте проект и нажмите `Показать скрытые`, если проект новый и все строки пока пустые.
8. Заполните `hostname`, `OS`, `type`, `comment` или `tags` у нескольких адресов.
...
12. Выберите несколько строк чекбоксами и проверьте массовое редактирование.
13. Проверьте поиск через верхнюю форму.
14. Для проекта или папки откройте расписание `Ping`, измените интервал или включите `Запустить сейчас`.
15. Проверьте `docker compose logs -f worker`: worker должен забирать задачи из очереди.
16. Если задан `INTEGRATION_API_TOKEN`, проверьте `GET /api/v1/projects` с заголовком `X-API-Key`.
17. Выполните `scripts/backup_postgres.sh` и убедитесь, что появился файл в `backups/`.
18. Откройте `http://localhost:8000/health` и убедитесь, что вернулся статус `ok`.
